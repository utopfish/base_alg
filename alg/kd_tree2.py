...
class Node(object):
    ...
    # __str__ 打印时才会显示 重构方法的内容，__repr__ 在打印和调试时都会显示重构方法的内容
    def __repr__(self):
        return '<%(cls)s - %(data)s>' % \
            dict(cls=self.__class__.__name__, data=repr(self.data))

    # 用于将类转为布尔值，这里是判断Node中data是否为None
...
def check_dimensionality(point_list, dimensions=None):
    """Make sure all  points have same dimensions,and return dimensions
    :param point_list{List}:
